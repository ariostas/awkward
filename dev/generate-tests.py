...
def gencudaconftest(tests_dir):
    """Generate conftest.py with a session-scoped warm-up fixture for cuda.compute JIT kernels.

    The reducer kernels in awkward._connect.cuda._compute are JIT-compiled via
    cuda.compute (Numba CUDA) on first call.  Each unique (kernel, result_dtype,
    input_dtype) combination triggers a separate multi-second compilation.
    Running every combination once in a session-scoped autouse fixture fills the
    in-process Numba cache before any generated test starts, so the test suite
    itself runs without extra compilation overhead.
    """
...
def _warmup_cuda_compute_kernels():
    ...
    for result_t, input_t in [
        (cp.int32,  cp.int8),   (cp.int32,  cp.int16),  (cp.int32,  cp.int32),
        (cp.int64,  cp.int8),   (cp.int64,  cp.int16),  (cp.int64,  cp.int32),  (cp.int64,  cp.int64),
        (cp.uint32, cp.uint8),  (cp.uint32, cp.uint16), (cp.uint32, cp.uint32),
        (cp.uint64, cp.uint8),  (cp.uint64, cp.uint16), (cp.uint64, cp.uint32), (cp.uint64, cp.uint64),
        (cp.float32, cp.float32), (cp.float64, cp.float64),
    ]:
    ...
    # ------------------------------------------------------------------
    for input_t in [
        cp.int8, cp.int16, cp.int32, cp.int64,
        cp.uint8, cp.uint16, cp.uint32, cp.uint64,
        cp.float32, cp.float64, cp.bool_,
    ]:
...
"""
    ...
    gencudaconftest(tests_cuda_kernels)
...
"""
    ...
    gencudaconftest(unit_tests_cuda_kernels)
